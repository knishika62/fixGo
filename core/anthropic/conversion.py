...
def _anthropic_image_to_openai(block: Any) -> dict[str, Any]:
    ...
    if source_type == "base64":
        ...
        if not url:
            raise OpenAIConversionError(
                f"Unsupported image source type for OpenAI chat conversion: {source_type!r}"
            )
...
class AnthropicToOpenAIConverter:
    ...
    def _convert_user_message_with_injection(
        ...
        def flush_user() -> None:
            ...
            if not user_blocks:
                return
            if len(user_blocks) == 1 and user_blocks[0]["type"] == "text":
                result.append({"role": "user", "content": user_blocks[0]["text"]})
            else:
                result.append({"role": "user", "content": list(user_blocks)})
            user_blocks.clear()
        ...
        for block in content:
            ...
            if block_type == "text":
                ...
                flush_text()
                user_blocks.append(_anthropic_image_to_openai(block))
